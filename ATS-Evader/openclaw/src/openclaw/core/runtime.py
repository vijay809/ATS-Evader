...
class Runtime:
    def __init__(self, settings: RuntimeSettings) -> None:
        ...
        self.document_repository = SQLiteDocumentRepository(settings.database_path)
        self.tasks = TaskManager(self.events, self.task_repository)
        self.plugins = PluginManager(PluginContext(settings, self.events, self.tasks, self.document_repository, self.services))
