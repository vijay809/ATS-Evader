...
async def test_plugins_have_a_managed_lifecycle(tmp_path: Path) -> None:
    ...
    events = EventBus()
    context = PluginContext(RuntimeSettings(), events, TaskManager(events, repository), ServiceRegistry())
...
async def test_plugin_lifecycle_events_are_published(tmp_path: Path) -> None:
    ...
    manager = PluginManager(
        PluginContext(RuntimeSettings(), events, TaskManager(events, repository), ServiceRegistry())
    )
    received: list[str] = []
    ...
    events.subscribe("plugin.started", record)  # type: ignore[arg-type]
    events.subscribe("plugin.stopped", record)  # type: ignore[arg-type]
